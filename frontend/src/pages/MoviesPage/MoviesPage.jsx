...
function MoviesPage() {
  const { setMessageMovies, setIsUpdated, setIsLoading } =
    useContext(GeneralContext);
  const [isMoviesLoading, setIsMoviesLoading] = useState(false);
  const [moviesToShow, setMoviesToShow] = useState(
    JSON.parse(localStorage.getItem("movies-to-show")) || []
  );
  const [isChecked, setIsChecked] = useState(
    JSON.parse(localStorage.getItem("short-films")) || false
  );

  function filterMovies(isChecked, dataFiltered) {
    setMoviesToShow(
      isChecked ? dataFiltered.filter((el) => el.duration <= 40) : dataFiltered
    );
  }

  function handleFilteredMovies(data, query) {
    const dataFiltered = data.filter(
      (el) => compareSearch(el.nameRU, query) || compareSearch(el.nameEN, query)
    );
    filterMovies(isChecked, dataFiltered);
    localStorage.setItem("movies-to-show", JSON.stringify(dataFiltered));
    if (dataFiltered.length === 0) {
      setMessageMovies("Nothing found.");
    }
  }
  ...
  return (
    <main>
      ...
      <section className="movies-page">
        ...
        />
        {isMoviesLoading ? <Preloader /> : <Movies movies={moviesToShow} />}
      </section>
    </main>
  );
}
