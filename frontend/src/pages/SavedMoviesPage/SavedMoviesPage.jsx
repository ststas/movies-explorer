...
function SavedMoviesPage () {
  ...
  const { 
    savedMovies, setMessageSavedMovies, 
    savedMoviesToShow, setSavedMoviesToShow, 
  } = useContext(GeneralContext);
  const [isChecked, setIsChecked] = useState(false)
  const [searchedMovies, setSearchedMovies] = useState(savedMovies)

  useEffect(() => {
    savedMoviesToShow.length === 0 ? setMessageSavedMovies("Ничего не найдено.") : setMessageSavedMovies("")
  ...
  useEffect(() => {
    ...
    setSearchedMovies((prevSearchedMovies) =>
      prevSearchedMovies.length === 0
        ...
        : savedMovies.filter(
            (el) =>
            prevSearchedMovies.findIndex((m) => m.movieId === el.movieId) !== -1,
          ),
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [savedMovies]);
  ...
  useEffect(()=> {
    setSavedMoviesToShow(JSON.parse(localStorage.getItem('saved-movies-to-show')) || savedMovies)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  },[])
  ...
  function handleSearchedMovies (data, query) {
    const dataFiltered = data.filter((el) => {
      if(isChecked) {
        return (el.duration <=40 && (compareSearch(el.nameRU, query) || compareSearch(el.nameEN, query)))
      }
